...
#"$(cat /etc/issue | head -1 | cut -f1-3 -d" ") / $(cat /etc/dgx-release | grep -E "DGX_PRETTY_NAME|DGX_OTA_VERSION" |cut -f2 -d= |cut -f2 -d '"' |paste -sd' ')"
echo "${MLPERF_HOST_OS}"
...
for nrun in `seq 1 $NEXP`; do
  ...
  for hostn in ${hosts[@]}; do
      if [[ $INSLURM -eq 1 ]]; then
        ...
        podman run --rm --privileged --entrypoint bash $CONT -c "sync && echo 3 > /proc/sys/vm/drop_caches && python3 -c $LOG_COMPLIANCE || exit 1" &
      fi
    pids+=($!); rets+=($?);
  done
  wait "${pids[@]}"
  ...
  for h in `seq 0 $((SLURM_NNODES-1))`; do
    ...
    if [[ $SLURM_NNODES -gt 1 ]]; then
      ...
      # Execute command
      set -x
      $(eval echo $SRUN) podman exec "${DOCKERENV[@]}" cont_${SLURM_JOB_ID} ./run_and_time.sh &
    pids+=($!);
      set +x
  done
  wait "${pids[@]}"
