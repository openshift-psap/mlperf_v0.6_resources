...
BENCHMARK_NAME="TRANSFORMER"
CONT=${CONT:-"mlperf-nvidia:$BENCHMARK"}
DATADIR=${DATADIR:-"/raid/data/xformer_v0p6/utf8"}
...
#DOCKEREXEC="env NV_GPU=${NV_GPU} nvidia-podman run --init --rm --net=host --uts=host --ipc=host --ulimit stack=67108864 --ulimit memlock=-1 --name=cont_${SLURM_JOB_ID} --security-opt seccomp=unconfined $IBDEVICES"
DOCKEREXEC="env NV_GPU=${NV_GPU} podman run --rm --net=host --uts=host --ipc=host --ulimit stack=67108864 --ulimit memlock=-1 -e NVIDIA_VISIBLE_DEVICES=ALL --cap-drop=ALL --security-opt label=type:nvidia_container_t --security-opt seccomp=unconfined --name=cont_${SLURM_JOB_ID} $IBDEVICES"

## Get version of the OS
export MLPERF_HOST_OS=/etc/redhat-release
...
for hostn in ${hosts[@]}; do
  timeout -k 600s 600s \
    $(eval echo $SRUN) $DOCKEREXEC $CONT python3 -c 'import torch; print("Found",torch.cuda.device_count(),"CUDA GPUs")' &
  pids+=($!);
done
wait "${pids[@]}"
...
pids=(); rets=()
for hostn in ${hosts[@]}; do
  $(eval echo $SSH) podman exec cont_${SLURM_JOB_ID} rm -f /etc/shinit &
  pids+=($!);
done
wait "${pids[@]}"
...
for nrun in `seq 1 $NEXP`; do
  (
    ...
    if [[ $SYSLOGGING -eq 1 ]]; then
        VARS_STR="${VARS[@]}"
        bash -c "echo -n 'Gathering sys log on ' && hostname && podman exec $VARS_STR cont_${SLURM_JOB_ID} python3 -c ${SYS_LOG_GET}"
        if [[ $? -ne 0 ]]; then
    ...
    for hostn in ${hosts[@]}; do
      if [[ $INSLURM -eq 1 ]]; then
        ...
        $(eval echo $SSH) podman exec cont_${SLURM_JOB_ID} python3 -c $LOG_COMPLIANCE &
      else
        podman run --init --rm --privileged --entrypoint bash $CONT -c "sync && echo 3 > /proc/sys/vm/drop_caches && python3 -c $LOG_COMPLIANCE || exit 1" &
      fi
      pids+=($!); rets+=($?);
    done
    wait "${pids[@]}"
    ...
    for h in `seq 0 $((SLURM_NNODES-1))`; do
      ...
      # Execute command
      set -x
      $(eval echo $SRUN) podman exec "${DOCKERENV[@]}" -e MODE=TRAIN cont_${SLURM_JOB_ID} ./run_and_time.sh &
      pids+=($!);
      set +x
    done
    wait "${pids[@]}"
...
if [[ $INSLURM -eq 0 ]]; then
  podman rm -f cont_${SLURM_JOB_ID}
fi
